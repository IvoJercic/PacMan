...
patches-own [ pellet-grid? oznaka istrazen? brPosjeta]  ;; true/false: is a pellet here initially?
...
to new  ;; Observer Button
  ...
  if brNeprijatelja = "0"[ask ghosts [die]]
  if brNeprijatelja = "1"[repeat 3 [ask one-of ghosts[die]]]
  if brNeprijatelja = "2"[repeat 2 [ask one-of ghosts[die]]]


  ;;Bojanje rubnih zidova
  ...
  ask patches with [pcolor = blue][set istrazen? true]
  ask patches with [pcolor = black]
  [
    set oznaka  (pxcor * 100 + pycor)
    set plabel oznaka
    set istrazen? false
    set brPosjeta 0
  ]
  ...
  [
    set tempLista lput (oznaka) tempLista
  ]
  set lista sort-by < tempLista

  ask patch 0 -5 [  set istrazen? true]
  set otvoreni []
  set otvoreni lput [oznaka] of pacman 0 otvoreni
  ask patches with [pcolor = blue] [set istrazen? true]
  ask patches with [pcolor = black and pellet-grid? = false][set istrazen? true]

  set brojKolacica count patches with [pellet-grid? = true and pcolor = black]


  ask ghosts
    [
      set koordDuhovi lput xcor koordDuhovi
      set koordDuhovi lput ycor koordDuhovi
    ]
...
to load-map
  ...
  let maps ["pacmap3.csv" ]
  if LAB = "1" [set maps ["pacmap1.csv" ]]
  if LAB = "2" [set maps ["pacmap2.csv" ]]
  if LAB = "3" [set maps ["pacmap3.csv" ]]
  ...
  [ import-world item (level - 1) maps
    ...
    set dead? false
    ask pacmans
    [ set home-pos list xcor ycor   set heuristika 0  set pen-mode "down"   set pojedenihKolacica 0]
    ask ghosts
    [ set home-pos list xcor ycor ]
  ]
  [ set level 1
    load-map ]
    if LAB = "3"[ask pellets with [xcor = 0 and ycor = -9 ][set shape "circle" set powerup? true]]
end


to play
  ;; brojKolacica -1 da se ne ulazi u novi ciklus
  if [lives = 0] of pacman 0 or [pojedenihKolacica = brojKolacica - 1 ] of pacman 0
  [ stop ]
  move-pacman
...
to consume
  ;;Ako pojedemo veliki kolacic duh se poplasi
  if any? pellets-here
...
to choose-heading  ;; Ghosts Procedure
  ...
  ;;Ako je samo jedan smjer slobodan
  if length dirs = 1
  [ set heading item 0 dirs ]

  ;;ako su dva smjera slobodna,ajde tamo di se vidi pacman, ako se ne vidi aj bilo di
  if length dirs = 2
...
;;Provjerava vidi li duh pacmana
...
algoritam
algoritam
"pretraga-po-id-patcha" "pretraga-po-posjecenosti" "pohlepni-algoritam" "astar" "pretraga-po-sirini"
3
